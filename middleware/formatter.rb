    ...
    def call(env)
      ...
      if !headers.include?('Content-Type') || headers['Content-Type'] != ''
        ...
        body_str, ctype = case return_type
          when 'json' then [self.to_json(body)       , 'application/json']
          when 'jsonp' then [self.to_jsonp(body, env) , 'text/javascript']
          when 'xml' then [self.to_xml(body)        , 'text/xml']
          when 'csv' then [self.to_csv(body)        , 'text/csv']
          when 'ics' then [self.to_ics(body)        , 'text/ics']
          when 'vcard' then [self.to_vcard(body)      , 'text/vcard']
          when 'txt' then [self.to_txt(body)        , 'text/plain']
          when 'html' then [self.to_html(body)       , 'text/html']
          else               [self.to_json(body)       , 'application/json']
        end
