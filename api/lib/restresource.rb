...
module MojuraAPI
  ...
  class RestResource
    ...
    def required_params_present?(method, params)
      conds = case method
        when :all    then self.all_conditions
        when :put    then self.put_conditions
        when :get    then self.get_conditions
        when :post   then self.post_conditions
        when :delete then self.delete_conditions
        else
      end
