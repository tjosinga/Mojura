  ...
  class DbObject
    ...
    def save_to_db
      is_new = self.id.nil?
			...
			data.stringify_keys!
			if is_new
				@id = @object_collection.insert(data, {safe: true}).to_s
			else
				@object_collection.update({_id: BSON::ObjectId(self.id)}, {'$set' => data}, {safe: true}) if (!data.empty?)
			end
    ...
    # :category: Database methods
    def delete_from_db
      @object_collection.remove({_id: BSON::ObjectId(self.id)}, {safe: true})
      @id = nil
			@options[:tree].new.refresh if @options.has_key?(:tree)
      return self
    end
