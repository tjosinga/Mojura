...
require 'test/unit'
require 'lib/mojura/api/lib/validator'
...
module MojuraAPI
	...
	class ValidatorTester < Test::Unit::TestCase
		...
		def test_is_website
			assert(Validator.is_url?('http://www.osingasoftware.nl'))
			assert(Validator.is_url?('http://www.cmd.tech.nhl.nl'))
			assert(Validator.is_url?('http://www.cmd.tech.nhl.nl/?test=sadlifjamwlrjasvlusmvietia&amp;afdsasdfasl.kjfasfasdf'))
			assert(!Validator.is_url?('www.osingasoftware.nl'))
			assert(!Validator.is_url?('taco@osisoft.nl'))
			assert(!Validator.is_url?(123))
			assert(!Validator.is_url?(true))
			assert(!Validator.is_url?(nil))
		...
		def test_is_time
			assert(Validator.is_time?('00:00'));
			assert(Validator.is_time?('12:12'));
			assert(Validator.is_time?('23:00'));
			assert(Validator.is_time?('23:00'));
			assert(Validator.is_time?('23:59'));
			assert(Validator.is_time?('23:59:59'));
			assert(!Validator.is_time?('24:00'));
			assert(!Validator.is_time?('23:59:60'));
			assert(!Validator.is_time?('0:0'));
			assert(!Validator.is_time?('0:60'));
			assert(!Validator.is_time?('0:600'));
			assert(!Validator.is_time?('35:600'));
			assert(!Validator.is_time?('12:0:00'));
			assert(!Validator.is_time?('12:10:0'));
		end

		def test_is_date
			assert(Validator.is_date?('2014-01-01'));
			assert(Validator.is_date?('0000-01-01'));
			assert(Validator.is_date?('9999-12-31'));
			assert(Validator.is_date?('2014-9-12'));
			assert(Validator.is_date?('2014-9-30'));
			assert(!Validator.is_date?('9999-13-31'));
			assert(!Validator.is_date?('2014-9-31'));
		end
	end
