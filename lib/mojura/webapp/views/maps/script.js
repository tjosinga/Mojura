var MapsView = (function($) {
	...
	function loadLocations(mapId, zoomIn) {
	...
	function addMarker(mapId, location) {
		...
		var marker = L.marker([location.latitude, location.longitude]).addTo(maps[mapId].map);
	...
	function addEditLocation(mapId, locationid) {
		Modal.create({
			id: "mapsAddEditModal",
			modal_large: true,
			title: Locale.str('maps', 'edit'),
			oncreated: function(modalId) {
				...
				$.getJSON(url, function(data) {
					...
					data.method = (locationid === "new") ? "put" : "post";
					if (locationid === "new") {
