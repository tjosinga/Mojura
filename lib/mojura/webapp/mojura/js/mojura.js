...
var Modal = (function($) {
	...
	function create(options) {
		Locale.ensureLoaded("system", { loaded: function() {
			...
			$("#" + options.id).one("hidden.bs.modal", function(elem) {
				$("#" + options.id).remove();
			}); //Self-destruct
...
var PageEditor = (function ($) {
	...
	function showAddPage(pageid) {
		...
		$("#modalAddPage form .elements").html(html);
		$("#modalAddPage form input[name=title]").val("");
		$("#modalAddPage form input[name=parentid]").val(pageid);
		$("#modalAddPage").modal("show");
		$("form", "#modalAddPage").on("submit", function () {

			$(this).ajaxSubmit({success: function () {
				var title = $("input[name=title]", "#modalAddPage").val();
				var url = "";
				if (pageid !== "") {
				...
				url = url.replace(/\/+$/, '') + "/" + encodeURIComponent(title) + "#editing";
				alert(url);
...
var TextEditor = (function($) {
	...
	function init(textareaSelector, parentModal) {
		addToolbar(textareaSelector, parentModal);
	}
...
//			textareaSelector = $(this).parents(".texteditor_toolbar").attr("data-selector");
	...
	function loadModals() {
		if ($("#texteditor_modals").length === 0) {
			...
			$.get(url, {cache: false}, function (template) {
				var html = Mustache.to_html(template, Locale.getViewsStrings(["system"]));
